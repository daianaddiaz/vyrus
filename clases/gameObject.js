export default class GameObject {
  ...
  async init() {
    ...
    this.juegoPausadoPorLogin = true;
    ...
    this.esperandoArranque = true; 
    this.tiempoEsperaInicial = 2000;
  ...
  update() {
    ...
    if (this.juegoPausadoPorLogin) {
      
      for (let i = 0; i < this.gameObjects.length; i++) {
        const obj = this.gameObjects[i];
        if (obj.container && obj.posicion) {
          obj.container.x = obj.posicion.x;
          obj.container.y = obj.posicion.y;
          obj.container.zIndex = Math.floor(obj.posicion.y);
        }
      }
